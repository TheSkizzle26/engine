import engine
import random


class Main(engine.Program):
    def __init__(self):
        super().__init__()
        engine.init(
            (1920, 1080),
            "pixel grass test",
            internal_size=(1920//4, 1080//4),
            input_map_path="assets/input_map.json",
            image_path="assets/images",
            flags=engine.ConfigFlags.FLAG_FULLSCREEN_MODE
        )
        engine.hide_cursor()

        self.display = engine.load_render_texture(1920//4, 1080//4)

        self.camera = engine.Camera((0, 0), 1, 2)

        self.grass_assets = engine.FoliageAssets(
            compute_ao=False,
            shadow_size=(24, 12),
            shadow_darkness=0.45,
        )
        self.grass_assets.add_image(engine.assets.load_texture_group("grass"), use_center_as_origin=True)
        self.grass = engine.FoliageManager(
            self.grass_assets,
            adaptivity=15,
            wind_force=30,
            shadows=True
        )

        self.cursor_size = 6
        self.last_mouse_world_pos = [0, 0]
        self.mouse_movement = 0

        self.num_blades = 0

    def update(self):
        if engine.input.is_pressed("quit"):
            self.quit()

        self.cursor_size += int(engine.get_mouse_wheel_move() * 3)

        m_pos = engine.get_internal_mouse_pos()
        c_pos = self.camera.get_world_topleft()
        pos = (
            c_pos[0] + m_pos[0] / self.camera.get_raylib_zoom(),
            c_pos[1] + m_pos[1] / self.camera.get_raylib_zoom()
        )

        self.mouse_movement += engine.vector2_distance(
            pos,
            self.last_mouse_world_pos
        )

        if engine.input.is_down("log") and self.mouse_movement > 3:
            self.mouse_movement = 0

            random_offset = self.cursor_size*3

            for i in range(self.cursor_size // 3):
                blade_pos = (
                    pos[0] + random.random() * random_offset - random_offset / 2,
                    pos[1] + random.random() * random_offset - random_offset / 2,
                )

                self.grass.spawn_object(blade_pos, random.randint(0, 5))
                self.num_blades += 1

        self.grass.prepare_update()
        self.grass.apply_force(pos, self.cursor_size * 6, 60)
        self.grass.update()

        self.last_mouse_world_pos = pos

        engine.set_window_title(f"FPS: {engine.get_fps()}")

    def render(self):
        engine.begin_texture_mode(self.display)
        engine.clear_background((23, 70, 36, 255))

        self.camera.begin()
        self.grass.render()
        self.camera.end()

        engine.draw_text(f"{self.num_blades} blades", 1, 0, 10, engine.WHITE)
        engine.draw_text(f"FPS: {engine.get_fps()}", 1, 8, 10, engine.WHITE)

        texture = engine.assets.load_texture("cursor.png")
        cursor_pos = (
            engine.get_internal_mouse_x() - texture.width / 2 * self.cursor_size / 3,
            engine.get_internal_mouse_y() - texture.height / 2 * self.cursor_size / 3
        )
        engine.draw_texture_ex(texture, cursor_pos, 0, self.cursor_size / 3, engine.WHITE)
        engine.end_texture_mode()

        engine.begin_drawing()
        engine.clear_background(engine.BLACK)
        engine.draw_texture_pro(
            self.display.texture,
            (0, 0, self.display.texture.width, -self.display.texture.height),
            (0, 0, engine.data.screen_size[0], engine.data.screen_size[1]),
            (0, 0),
            0,
            engine.WHITE
        )
        engine.end_drawing()


if __name__ == "__main__":
    Main().run()